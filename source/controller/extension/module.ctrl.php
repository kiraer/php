 ...
 * $sn: origins/source/controller/extension/module.ctrl.php : v bfe69d163d9f : 2014/08/20 10:16:07 : yanghf $
...
if($do == 'permission') {
	...
	if(empty($module)) {
		...
		message('你访问的模块不存在. 或许你愿意去微动力云服务平台看看. ', 'http://addons.we7.cc/search.php?q=' . $id);
...
if($do == 'install') {
	...
	$manifest = ext_module_manifest($id, true);
	...
	foreach($points as $p => $row) {
		...
		if(is_array($bindings[$p]) && !empty($bindings[$p])) {
			...
			if(!empty($delete_do)) {
				...
				pdo_query('DELETE FROM ' . tablename('modules_bindings') . " WHERE module = :module AND entry = :entry AND do NOT IN ('" . implode("','", $delete_do). "')", array(':module' => $manifest['application']['identifie'], ':entry' => $p));
			...
			if(!empty($delete_title)) {
				...
				pdo_query('DELETE FROM ' . tablename('modules_bindings') . " WHERE module = :module AND entry = :entry AND title NOT IN ('" . implode("','", $delete_title). "')", array(':module' => $manifest['application']['identifie'], ':entry' => $p));
			...
			if(!empty($delete_call)) {
				...
				pdo_query('DELETE FROM ' . tablename('modules_bindings') . " WHERE module = :module AND call NOT IN ('" . implode("','", $delete_call). "')", array(':module' => $manifest['application']['identifie']));
