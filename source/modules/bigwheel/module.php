...
class BigwheelModule extends WeModule {
    ...
    public function doawardlist() {
        ...
        $total = pdo_fetchcolumn("SELECT count(a.id) FROM " . tablename('bigwheel_award') . " a left join " . tablename('bigwheel_fans') . " f on a.fansID=f.id WHERE a.rid = :rid and a.weid=:weid " . $where . "", $params);
        $pindex = max(1, intval($_GPC['page']));
        $psize = 12;
        $pager = pagination($total, $pindex, $psize);
        $start = ($pindex - 1) * $psize;
        $limit .= " LIMIT {$start},{$psize}";
        $list = pdo_fetchall("SELECT a.* FROM " . tablename('bigwheel_award') . " a left join " . tablename('bigwheel_fans') . " f on a.fansID=f.id WHERE a.rid = :rid and a.weid=:weid  " . $where . " ORDER BY a.id DESC " . $limit, $params);
